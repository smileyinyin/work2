  ...
  <div class="user-detail">
    ...
    <Card>
      <Form :label-width="100">
        ...
        <Row class="mt20">
          <i-col span="12">
            <FormItem label="会员ID：">{{params.mobile}}</FormItem>
            <FormItem label="昵称：">{{params.nickname}}</FormItem>
            <FormItem label="状态：">
            ...
            <FormItem label="身份证号：">{{params.identityCard}}</FormItem>
          </i-col>
          <i-col span="12">
        ...
        <Row class="mt20">
          <i-col span="12">
            <FormItem label="余额：">{{params.balance}}</FormItem>
            <!-- <Button type="error" @click="jf"  size="small" style="margin-left: 20px;">积分明细</Button> -->
            <FormItem label="积分：">{{params.integral}}</FormItem>
            <FormItem label="线上订单：">{{params.orderCountOnline}}</FormItem>
            <FormItem label="线下订单：">{{params.orderCountOffline}}</FormItem>
            <FormItem label="绑定门店：">{{params.province}} {{params.city}} {{params.district}} {{params.name}}</FormItem>
          </i-col>
          <i-col span="12">
            <FormItem label="绑定银行卡：">
              <span>{{params.bankCards}}</span>
              <!-- <Button type="error" @click="untie(item, index)" size="small" style="margin-left: 20px;">解绑</Button> -->
            </FormItem>
            <FormItem label="绑定分公司：">
              <span>{{params.company}}</span>
            </FormItem>
            <FormItem label="绑定体验店：">
              <span>{{params.experienceStoreName}}</span>
            </FormItem>
          </i-col>
        </Row>
      </Form>
...
<script>
import { findUser } from '@/api/member/index'
...
export default {
  ...
  created() {
    ...
    findUser({ userId: this.id }).then(res => {

...
<script>
import { getFirstDayOfMonth, getDayCountOfMonth, deepCopy } from '@/libs/util'
...
export default {
  // mixins: [Locale],  // 这里的 i18 会跟main.js里的 i18 冲突导致报错，由于这里只有三个地方用到国际化（this.t方法），直接手动转换就好
  props: {
    date: {},
    year: {},
    month: {},
  ...
  computed: {
    ...
    headerDays () {
      const weekStartDay = 0 // Number(this.t('i.datepicker.weekStartDay'))
      const translatedDays = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'].map(item => {
        return ({
          ...
          fri: '五',
          sat: '六'
        })[item] // this.t('i.datepicker.weeks.' + item)
      })

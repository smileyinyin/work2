  ...
  <Dropdown @on-click="handleClick" transer placement="right-start">
    ...
    <DropdownMenu slot="list">
      <template v-for="child in children">
        ...
        <DropdownItem v-else :key="`drop-${child.name}`" :name="getNameOrHref(child, child.children)"><common-icon :size="iconSize" :type="child.icon"/><span class="menu-title">{{ showTitle(child) }}</span></DropdownItem>
      </template>
    </DropdownMenu>
  </Dropdown>
</template>

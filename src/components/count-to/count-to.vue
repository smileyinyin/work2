<template>
  <div class="count-to-wrapper">
    ...
    <p class="content-outer">
      <slot name="prefix"></slot><span :class="['count-to-count-text', countClass]" :id="counterId">{{ init }}</span><i :class="['count-to-unit-text', unitClass]">{{ unitText }}</i>
    </p>
    <slot name="right"/>
  </div>
</template>
...
export default {
  ...
  watch: {
      ...
      this.counter && this.counter.update(endVal)
    }
  }
}
</script>
